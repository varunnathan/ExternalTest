...
from src.modelling.utils.logging_utils import logger, init_logger
from src.modelling.model.product_ranker import ProductRanker
from src.modelling.model.optimizer import build_optim
from src.modelling.model.item_transformer import ItemTransformerRanker
from src.modelling.data.product_search_data_preparation import GlobalProdSearchData, ProdSearchData
...
def parse_args():
    ...
    parser.add_argument("--use_item_pos", type=str2bool, nargs='?', const=True, default=False,
                        help="use the embeddings corresponding to a candidate item as an output when encoding the \
                        purchased item sequence.")
    ...
    parser.add_argument("--do_subsample_mask", type=str2bool, nargs='?', const=True, default=False,
                        help="do subsampling mask do the reviews with cutoff review_word_limit; otherwise do subsampling \
                        then do the cutoff.")
    ...
    parser.add_argument("--test_candi_size", type=int, default=-1,
                        help="When it is 0 or less, all the products are used. Otherwise, test_candi_size samples \
                        from ranklist will be reranked")
    ...
    parser.add_argument("--corrupt_rate", type=float, default=0.9,
                        help="the corruption rate that is used to represent the paragraph in the corruption module.")
    parser.add_argument("--shuffle_review_words", type=str2bool, nargs='?', const=True, default=True,
                        help="shuffle review words before collecting sliding words.")
    parser.add_argument("--train_review_only", type=str2bool, nargs='?', const=True, default=True,
                        help="whether the representation of negative products need to be learned at each step.")

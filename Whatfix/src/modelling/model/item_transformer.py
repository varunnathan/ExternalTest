...
from src.modelling.model.text_encoder import AVGEncoder, FSEncoder, get_vector_mean
from src.modelling.model.transformer import TransformerEncoder
from src.modelling.model.transformer_modules import MultiHeadedAttention
from src.modelling.utils.logging_utils import logger
from src.modelling.utils.loading_utils import load_pretrained_embeddings
...
class ItemTransformerRanker(nn.Module):
    def __init__(self, args, device, vocab_size, product_size, vocab_words, word_dists=None):
        ...
        if self.pretrain_emb_dir is not None:
            ...
            word_index_dic, pretrained_weights = load_pretrained_embeddings(pretrain_word_emb_path)
    ...
    def forward_trans(self, batch_data):
        query_word_idxs = batch_data.query_word_idxs
    ...
    def forward_attn(self, batch_data):
        query_word_idxs = batch_data.query_word_idxs
        target_prod_idxs = batch_data.target_prod_idxs
        u_item_idxs = batch_data.u_item_idxs
        batch_size, _ = u_item_idxs.size()
    ...
    def forward_dotproduct(self, batch_data):
        query_word_idxs = batch_data.query_word_idxs
        target_prod_idxs = batch_data.target_prod_idxs
        u_item_idxs = batch_data.u_item_idxs
        batch_size, prev_item_count = u_item_idxs.size()
    ...
    def forward_seq(self, batch_data):
        query_word_idxs = batch_data.query_word_idxs
        target_prod_idxs = batch_data.target_prod_idxs

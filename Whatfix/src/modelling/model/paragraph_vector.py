...
from src.modelling.model.text_encoder import get_vector_mean
...
class ParagraphVector(nn.Module):
    def __init__(self, word_embeddings, word_dists, review_count,
        ...
        if pretrain_emb_path is not None:
            _, pretrained_weights = load_pretrained_embeddings(pretrain_emb_path)
...
class ParagraphVectorCorruption(nn.Module):
    def __init__(self, word_embeddings, word_dists, corrupt_rate,
        ...
        if pretrain_emb_path is not None and vocab_words is not None:
            word_index_dic, pretrained_weights = load_pretrained_embeddings(pretrain_emb_path)

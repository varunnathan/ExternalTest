...
import numpy as np
import random
from src.modelling.utils.padding_utils import pad, pad_3d
from src.modelling.data.data_batching import ProdSearchTrainBatch, ProdSearchTestBatch
...
class ProdSearchDataLoader(DataLoader):
    ...
    def get_test_batch(self, batch):
        ...
        candi_prod_idxs = pad(candi_prod_idxs, pad_id = -1) #pad reviews
        candi_prod_ridxs = pad_3d(candi_prod_ridxs, pad_id = self.review_pad_idx, dim=1) #pad candi products
        candi_prod_ridxs = pad_3d(candi_prod_ridxs, pad_id = self.review_pad_idx, dim=2) #pad reviews of each candi
        candi_seg_idxs = pad_3d(candi_seg_idxs, pad_id = self.seg_pad_idx, dim=1)
        candi_seg_idxs = pad_3d(candi_seg_idxs, pad_id = self.seg_pad_idx, dim=2)
        candi_seq_user_idxs = pad_3d(candi_seq_user_idxs, pad_id = self.user_pad_idx, dim=1)
        candi_seq_user_idxs = pad_3d(candi_seq_user_idxs, pad_id = self.user_pad_idx, dim=2)
        candi_seq_item_idxs = pad_3d(candi_seq_item_idxs, pad_id = self.prod_pad_idx, dim=1)
        candi_seq_item_idxs = pad_3d(candi_seq_item_idxs, pad_id = self.prod_pad_idx, dim=2)
    ...
    def get_train_batch(self, batch):
        ...
        if len(query_word_idxs) == 0:
            ...
            return None
        pos_prod_ridxs = pad(pos_prod_ridxs, pad_id = self.review_pad_idx) #pad reviews
        pos_seg_idxs = pad(pos_seg_idxs, pad_id = self.seg_pad_idx)
        pos_user_idxs = pad(pos_user_idxs, pad_id = self.user_pad_idx)
        pos_item_idxs = pad(pos_item_idxs, pad_id = self.prod_pad_idx)
        ...
        #pos_prod_rword_idxs = pad(pos_prod_rword_idxs, pad_id = self.word_pad_idx)
        ...
        neg_prod_ridxs = pad_3d(neg_prod_ridxs, pad_id = self.review_pad_idx, dim=1) #pad neg products
        neg_prod_ridxs = pad_3d(neg_prod_ridxs, pad_id = self.review_pad_idx, dim=2) #pad reviews of each neg
        neg_seg_idxs = pad_3d(neg_seg_idxs, pad_id = self.seg_pad_idx, dim=1)
        neg_seg_idxs = pad_3d(neg_seg_idxs, pad_id = self.seg_pad_idx, dim=2)
        neg_user_idxs = pad_3d(neg_user_idxs, pad_id = self.user_pad_idx, dim=1)
        neg_user_idxs = pad_3d(neg_user_idxs, pad_id = self.user_pad_idx, dim=2)
        neg_item_idxs = pad_3d(neg_item_idxs, pad_id = self.prod_pad_idx, dim=1)
        neg_item_idxs = pad_3d(neg_item_idxs, pad_id = self.prod_pad_idx, dim=2)
        ...
        #neg_prod_rword_idxs = pad(neg_prod_rword_idxs, pad_id = self.word_pad_idx)
